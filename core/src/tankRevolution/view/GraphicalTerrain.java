...
public class GraphicalTerrain {
    ...
    private float radius = Constants.pixelsPerMeter();
    ...
    private  void setPoints() {
        ...
        for(Polygon polygon : polygons) {
            for (int i = 0; i < 150; i++) {
                for (int j = 0; j < 150; j++) {
                    float x = Constants.getMapWidth() / 150 * i;
                    float y = Constants.getMapWidth() / 150 * j;
                    if (polygon.contains(x, y)) {
                        pixmap.setColor(0,1,0,1);
                        pixmap.fillCircle((int)(x * pixelsToMeter), (int)(y * pixelsToMeter), (int)radius);
                    }
                }
            }
        }
